...
// Add keyboard shortcut listener for Ctrl+Q as fallback
document.addEventListener("keydown", (event) => {
  if ((event.ctrlKey || event.metaKey) && event.key === "q") {
    event.preventDefault();
    console.log("Ctrl+Q pressed (content script), triggering rephrase");
...
chrome.runtime.onMessage.addListener((message) => {
  if (message && message.type === "REPHRASE_REPLACE") {
    ...
    if (message.isHtml) {
      ...
      tempDiv.innerHTML = message.rephrasedText;
      plainText = tempDiv.textContent || tempDiv.innerText || message.rephrasedText;
      console.log("Parsed HTML to plain text:", plainText);
    }
